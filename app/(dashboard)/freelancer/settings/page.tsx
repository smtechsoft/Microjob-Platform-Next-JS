...
export default function FreelancerSettingsPage() {
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <PageHeader
        title="Settings"
        description="Manage your account preferences, security, and profile details."
      />
      ...
      <Tabs defaultValue="profile" className="w-full space-y-6">
        <div className="flex items-center justify-between">
          <TabsList className="bg-muted/40 p-1 rounded-xl border border-border/30 h-10 w-full lg:w-auto">
            ...
            <TabsTrigger value="notifications" className="text-xs font-bold uppercase tracking-wider rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground h-8 px-6 transition-all">
              Notifications
            </TabsTrigger>
          </TabsList>
        </div>
          ...
          <TabsContent value="password" className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
            <Card>
              <CardHeader>
                ...
                <CardDescription className="text-xs text-muted-foreground font-medium uppercase tracking-tight">Update credentials regularly.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                <div className="space-y-2">
          ...
          <TabsContent value="notifications" className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
            <Card>
              <CardHeader>
                ...
                <CardDescription className="text-xs text-muted-foreground font-medium uppercase tracking-tight">Notification alert controls.</CardDescription>
              </CardHeader>
                ...
                {[
                  ...
                  <div key={i} className="flex items-center justify-between py-2 border-b border-border/30 last:border-0 pb-4 last:pb-0">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[10px] text-muted-foreground italic">{item.description}</p>
                    </div>
                    <Switch defaultChecked={i < 3} className="data-[state=checked]:bg-primary" />
                  </div>
                ))}
                <Button className="w-full sm:w-auto h-10 px-8 rounded-lg bg-primary text-primary-foreground font-bold uppercase tracking-wider text-xs mt-4">
                  Save Preferences
                </Button>
              </CardContent>
            </Card>
